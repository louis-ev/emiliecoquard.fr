...
function onIconEvents( $this ) {
	...
	$this.on('click', function(e) {
		...
		(function polygonStrokeAnimation() {
			d3.select( ".conteneurProjet svg" ).select(".polygones")
				...
				.each( "end", function() {
					...
					if ( $('.offscreenDump article.dump').length ) {
		...
		function polygonesFillAnimation() {
			d3.select( ".conteneurProjet svg" ).select(".polygones")
				...
				.each( "end", function() {
					...
					var contents = $('.offscreenDump article.dump').html();
					$(".conteneurProjet article.projet").append(contents);

					$(".conteneurProjet").removeClass("is-loading").addClass("is-filling").find("svg").remove("svg");

					setTimeout(function() {
						$('body').removeClass('is-loading is-busy');
						$(".conteneurProjet").removeClass("is-filling").addClass("is-filled");
						$('.offscreenDump').removeClass("dump");
		...
		$('.offscreenDump').load(urlToLoad, "", function(){
			$('.offscreenDump article').removeClass("projet").addClass("dump");
		});
	});
}
...
$("document").ready( function() {
	...
	window.$itemList = $(".listCategories .itemList");
	...
	$(".listCategories .categorie").each(function() {
		...
		$this.on("click", function() {
			...
			var $projetFiltresSVG = $projetFiltres.find("svg");
 			...
 			//$listOrder.append ( $projetFiltresClones.filter(function() { return $(this).hasClass(".is-viewed") }) );
/*
 			console.log( "$listOrder " );
 			console.log( $listOrder );
 			console.log( $listOrder.find("is-viewed") );
*/
...
function closeProject() {
	...
	$itemList.removeClass("is-sticky");
	$borderTop.removeClass('is-sticky');

}

function fixedBorderTop (scrollfromtop) {

	if ( $("body").hasClass("is-project") ) {
/*
		if ( scrollfromtop > listCategoriesULPos ) {
			$itemList.addClass("is-sticky");
		} else {
			$itemList.removeClass("is-sticky");
		}
*/

		if ( scrollfromtop > borderTopPos ) {

			//$borderTop.addClass('is-sticky');
...
/*
		if ( scrollfromtop > borderBottomPos - 290 ) {

			$("#entete").addClass("long-sticky");

			$("body").addClass("is-busy");

			setTimeout(function() {
				$("#entete").removeClass("long-sticky");
				closeProject();
			}, 800)
...
*/
		...
		if ( scrollfromtop > borderBottomPos - 290 &&  scrollfromtop < borderBottomPos ) {

			var scrollUpCatBar = scrollfromtop - (borderBottomPos-290);
			console.log( " — scrollUpCatBar : " + scrollUpCatBar );
			$("#entete.is-sticky .conteneurEntete").transition({
				y: scrollUpCatBar,
				duration:0

				});

		} else if (  scrollfromtop > borderBottomPos ) {


			//closeProject();
		} else {

			$("#entete.is-sticky .conteneurEntete").css("transform", "translate(0px, 0px)");
